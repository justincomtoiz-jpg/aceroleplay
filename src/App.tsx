...
function App() {
  ...
  const [connectEndpoint, setConnectEndpoint] = useState('45.92.36.118:30125');
    ...
    <div className="min-h-screen relative overflow-hidden bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950">
      ...
      <div className="relative container mx-auto px-6 py-16 max-w-5xl">
        ...
        <div className="flex flex-wrap justify-center gap-8 mb-16">
          <button onClick={handleCopy} className="px-10 py-5 bg-cyan-600 hover:bg-cyan-500 rounded-xl font-bold text-lg flex items-center gap-4 transition transform hover:scale-105 shadow-lg">
            <LinkIcon className="w-8 h-8" />
            Copy Server Link
          </button>
          <button onClick={() => window.open(discordLink, '_blank')} className="px-10 py-5 bg-purple-600 hover:bg-purple-500 rounded-xl font-bold text-lg flex items-center gap-4 transition transform hover:scale-105 shadow-lg">
          ...
          {serverInfo.loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-xl">
              <div className="flex items-center gap-4">
                <div className={`w-6 h-6 rounded-full ${serverInfo.online ? 'bg-emerald-400 animate-pulse' : 'bg-red-500'}`}></div>
                <span className="font-semibold">{serverInfo.online ? 'Online' : 'Offline'}</span>
              </div>
              <div>Server: <span className="font-bold text-cyan-300">{serverInfo.hostname}</span></div>
              <div className="flex items-center gap-4">
        ...
        <div className="grid md:grid-cols-2 gap-12">
          ...
          <div className="bg-white/5 backdrop-blur-xl rounded-2xl p-10 border border-white/10">
            ...
            <a href="https://www.ace-roleplay.net/rules-hybrid.html" target="_blank" rel="noopener noreferrer" className="block mt-8 text-cyan-400 hover:text-cyan-300 text-lg">
              → Full Rules Here
            </a>
          </div>
        </div>
